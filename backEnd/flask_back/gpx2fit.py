...
def gpx2fit(gpx_file_path, sport=Sport.RUNNING, sub_sport=SubSport.GENERIC):
    ...
    with open(gpx_file_path,encoding='UTF-8') as Temp:
        gpx = gpxpy.parse(Temp)
    first_point_time = gpx.tracks[0].segments[0].points[0].time
    time_create = first_point_time.timestamp() * 1000
    ...
    out_path = "./download/output.fit"
    if os.path.isfile(out_path):
        print("delete old fit", out_path,)
        os.remove(out_path)
    if os.path.isfile(gpx_file_path):
        print("delete old gpx", gpx_file_path)
        os.remove(gpx_file_path)
    fit_file.to_file(out_path)
    return out_path

if __name__ == "__main__":
    gpx2fit("C:\\Users\\peter\\AppData\\Roaming\\Garmin\\VIRB Edit\\GMetrix\\ManualImport\\20240705.gpx")
